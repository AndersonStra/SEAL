...
def iterator_span(args):
    ...
    arg_it = _iterator_span_get_arguments(data, args.min_score, args.max_rank, args.mark_target, args.mark_silver,
                                          args.min_score_gold)
    arg_it = ((text, target, args.n_samples, args.min_length, args.max_length, args.temperature) for text, target in
              arg_it)

    with multiprocessing.Pool(args.jobs) as pool:
        for target, spans in pool.imap(extract_spans_wrapper, arg_it):
            for source in spans:
                if args.mark_target:
                    source += " || body"
                if args.mark_silver:
                    source += " || +"  # No description was found in the paper for mark_silver
                yield source, target
